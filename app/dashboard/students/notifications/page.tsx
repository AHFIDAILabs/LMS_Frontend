...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; bg: string; label: string }> = {
  announcement:       { icon: Megaphone,     color: "text-lime-400",    bg: "bg-lime-500/10 border-lime-500/30",      label: "Announcement" },
  assessment_due:     { icon: FileText,      color: "text-yellow-400",  bg: "bg-yellow-500/10 border-yellow-500/30",  label: "Assessment" },
...
export default function StudentNotificationsPage() {
  ...
  const [unreadCount, setUnreadCount] = useState(0);   // ✅ tracks unread specifically
  const limit = 15;

  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ Fetch paginated list + a separate unread-only count
      const [notifRes, unreadRes] = await Promise.all([
        studentService.getNotifications({ page, limit, unreadOnly }),
        studentService.getNotifications({ page: 1, limit: 1, unreadOnly: true }),
      ]);
      if (notifRes.success) { setRows(notifRes.data || []); setTotal(notifRes.total || 0); }
      // ✅ unread count from the total field of unread-only request
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || user?.role !== "student") { router.replace("/auth/login"); return; }
    void fetchAll();
  }, [authLoading, isAuthenticated, user?.role, fetchAll]);

  const markRead    = async (id: string) => { await studentService.markNotificationRead(id);    void fetchAll(); };
  const markAllRead = async ()           => { await studentService.markAllNotificationsRead();   void fetchAll(); };
  const remove      = async (id: string) => { await studentService.deleteNotification(id);       void fetchAll(); };
  ...
  const groups = filtered.reduce<Record<string, any[]>>((acc, n) => {
    ...
    if (d.toDateString() === now.toDateString()) {
      ...
      bucket = d.toDateString() === yesterday.toDateString()
        ? "Yesterday"
        ...
        <header className="mb-8">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-12 h-12 rounded-2xl bg-lime-500/10 border border-lime-500/20 flex items-center justify-center">
                  <Bell size={24} className="text-lime-400" />
                </div>
                {/* ✅ Live count badge on the page header bell */}
                {unreadCount > 0 && (
            ...
            <div className="flex gap-2">
              ...
              {unreadCount > 0 && (
                <button onClick={markAllRead}
                  className="px-4 py-2 bg-lime-600 hover:bg-lime-500 rounded-xl text-white text-sm font-medium flex items-center gap-2 transition-all">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            {[
              ...
              { label: "Unread", value: unreadCount, color: "text-lime-400",    bg: "from-lime-900/20 to-lime-900/10",        border: "border-lime-500/30" },
        ...
        <div className="bg-slate-800/40 border border-gray-700/50 rounded-2xl p-4 mb-6 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2.5 bg-slate-900 border border-gray-700 rounded-xl text-white text-sm placeholder-gray-500 focus:outline-none focus:border-lime-500/50 transition-all"
            />
            {search && (
          ...
          <label className="flex items-center gap-2.5 px-4 py-2.5 bg-slate-900 border border-gray-700 rounded-xl cursor-pointer hover:border-lime-500/50 transition-all">
            <input
              type="checkbox"
              checked={unreadOnly}
                      ...
                      return (
                        <li
                          ...
                          className={`group flex items-start gap-4 p-5 transition-all hover:bg-slate-800/60 ${
                            !n.isRead ? "border-l-2 border-lime-500" : "border-l-2 border-transparent"
                          }`}
                        >
                            ...
                            <div className="flex items-start justify-between gap-3 flex-wrap">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                                  ...
                                  {!n.isRead && <span className="w-2 h-2 rounded-full bg-lime-400 shrink-0" />}
                                </div>
                            ...
                            <div className="flex gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                              {!n.isRead && (
                                <button onClick={() => markRead(n._id)}
                                  className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700 hover:bg-lime-600/20 border border-gray-600 hover:border-lime-500/50 rounded-lg text-xs text-gray-300 hover:text-lime-300 transition-all">
