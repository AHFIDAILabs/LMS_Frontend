...
import {
  ...
  Filter, Search, BookOpen, FileText, Award,
    ...
    <div className="min-h-screen bg-slate-950 flex">
      ...
      <div className="flex-1 lg:ml-64 p-4 sm:p-6 lg:p-8 overflow-y-auto">
        ...
        <div className="bg-slate-800/40 border border-gray-700/50 rounded-2xl overflow-hidden">
            ...
            <ul className="divide-y divide-gray-700/50">
              {filtered.map(n => {
                ...
                return (
                  <li
                    key={n._id}
                    className={`group flex items-start gap-4 p-5 transition-all hover:bg-slate-800/50 ${
                      !n.isRead ? "border-l-2 border-purple-500" : "border-l-2 border-transparent"
                    }`}
                  >
                    {/* Type icon */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <span className="text-xs text-gray-600 whitespace-nowrap">{timeAgo(n.createdAt)}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!n.isRead && (
                          <button onClick={() => markRead(n._id)}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700 hover:bg-purple-600/30 border border-gray-600 hover:border-purple-500/50 rounded-lg text-xs text-gray-300 hover:text-purple-300 transition-all">
          ...
          {/* Pagination */}
          {pages > 1 && (
            <div className="px-5 py-4 border-t border-gray-700/50 flex items-center justify-between text-sm text-gray-400">
