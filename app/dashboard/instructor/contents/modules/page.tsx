...
export default function ModulesListPage() {
  ...
  const fetchModules = async () => {
    ...
    try {
      ...
      const res = await moduleService.getAllModulesAdmin(params);
      setModules(res?.data || []);
  ...
  const onTogglePublish = async (id: string) => {
    try {
      await moduleService.togglePublish(id);
      await fetchModules();
    } catch (err: any) {
          ...
          <header className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Manage all course modules</p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <Link
                href="/dashboard/instructor/content/modules/create"
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {modules.map((m) => (
                      <tr key={m._id} className="hover:bg-slate-700/30">
                        <td className="px-4 py-3 text-white">{m.title}</td>
                        <td className="px-4 py-3 text-gray-300">{m.order}</td>
                        <td className="px-4 py-3 text-gray-300">{m.type || "core"}</td>
