...
import { Video, Plus, RefreshCw, Search } from "lucide-react";
...
export default function LessonsListPage() {
  ...
  const togglePublish = async (lessonId: string) => {
    try {
      await axiosClient.patch(`/lessons/${lessonId}/publish`);
      await fetchLessons();
    } catch (err: any) {
        ...
        <div className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8">
          <header className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Manage all lessons across your modules</p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <Link
                href="/dashboard/instructor/content/lessons/create"
          ...
          <div className="bg-slate-800/50 border border-gray-700 rounded-xl overflow-hidden">
            {loading ? (
              ...
              <div className="p-8 text-center text-gray-400">No lessons found.</div>
            ) : (
                ...
                <table className="w-full">
                  <thead className="bg-slate-700/50 border-b border-gray-700">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Estimated (min)</th>
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {lessons.map((l) => (
                      <tr key={l._id} className="hover:bg-slate-700/30">
                        <td className="px-4 py-3 text-white">{l.title}</td>
                        <td className="px-4 py-3 text-gray-300">{l.type}</td>
                        <td className="px-4 py-3 text-gray-300">{l.estimatedMinutes}</td>
