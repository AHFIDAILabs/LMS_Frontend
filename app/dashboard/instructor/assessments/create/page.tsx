...
import { 
  ...
  Clock, Target, Wand2, CheckSquare, FileText,
  Code, Award
} from "lucide-react";
...
export default function CreateAssessmentPage() {
  ...
  // ✅ FIXED: Updated to match backend model
  const [formData, setFormData] = useState<ICreateAssessment>({
    courseId: "",
    title: "",
    description: "",  // ✅ Required in backend
    type: AssessmentType.QUIZ,
    passingScore: 70,
    duration: 60,
    attempts: 2,  // ✅ Backend default
    isPublished: false,  // ✅ Backend default
    isRequiredForCompletion: true,  // ✅ Backend default
    questions: [
      {
        questionText: "",
        type: QuestionType.MULTIPLE_CHOICE,  // ✅ Now "multiple_choice" 
        points: 5,
        options: ["", "", "", ""],
        correctAnswer: "0", 
        explanation: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ FIXED: Validate description is not empty
    if (!formData.description?.trim()) {
      setError("Please add a description for this assessment.");
      return;
    }
    ...
    try {
      const res = await assessmentService.admin.create(formData);
      if (res.success) {
        router.push("/dashboard/instructor/assessments");
      } else {
        setError(res.message || "Failed to save assessment.");
      }
    } catch (err: any) {
  ...
  // Helper function to get assessment type info
  const getTypeInfo = (type: AssessmentType) => {
    const config = {
      [AssessmentType.QUIZ]: {
        icon: CheckSquare,
        color: "blue",
        label: "Quiz",
        description: "Time-limited test with auto-grading"
      },
      [AssessmentType.ASSIGNMENT]: {
        icon: FileText,
        color: "purple",
        label: "Assignment",
        description: "Flexible tasks with varied questions"
      },
      ...
      <div className="flex-1 lg:ml-64 overflow-y-auto relative">
        ...
        <div className="sticky top-0 z-30 bg-slate-900/95 backdrop-blur-md border-b border-gray-800 px-8 py-4 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">New Assessment</h1>
              <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                <TypeIcon size={12} />
                {currentTypeInfo.label} • {currentTypeInfo.description}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-500 hover:to-pink-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm font-bold shadow-lg shadow-purple-500/20"
            >
              {isGenerating ? (
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <section className="bg-slate-800/40 border border-gray-800 rounded-2xl p-6">
                    ...
                    <input
                      ...
                      placeholder="Assessment Title (e.g., JavaScript Fundamentals Quiz)"
                    ...
                    <textarea
                      ...
                      placeholder="Add instructions or description for your students... *"
                ...
                <div className="space-y-6">
                  <div className="bg-slate-800/40 border border-gray-800 rounded-2xl p-6 space-y-6 sticky top-24">
                    ...
                    <div>
                        ...
                        {Object.values(AssessmentType).map((type) => {
                          ...
                          return (
                            ...
                            >
                              <Icon 
                                size={20} 
                                className={isSelected ? `text-${info.color}-400` : 'text-gray-400'} 
                              />
                      ...
                      <p className="text-xs text-gray-500 mt-2">{currentTypeInfo.description}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          formData.isPublished ? 'bg-emerald-500' : 'bg-gray-700'
                        }`}
                      >
                        <span
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData({...formData, isRequiredForCompletion: !formData.isRequiredForCompletion})}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          formData.isRequiredForCompletion ? 'bg-emerald-500' : 'bg-gray-700'
                        }`}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                            formData.isRequiredForCompletion ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
              ...
              <div className="bg-slate-800/40 border border-gray-800 rounded-3xl p-8">
                ...
                <div className="flex gap-6 text-sm flex-wrap">
                  ...
                  <div className="flex items-center gap-2 text-orange-400">
                    <RefreshCw size={16}/> {formData.attempts} {formData.attempts === 1 ? 'Attempt' : 'Attempts'}
                  </div>
                </div>
              </div>
              ...
              {formData.questions.map((q, i) => (
                <div key={i} className="bg-slate-800/20 border border-gray-800 rounded-2xl p-8">
                  ...
                  <h3 className="text-xl text-white font-medium mb-8">
                    {q.questionText || "Question text will appear here"}
                  </h3>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isGenerating}
                  className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Replace All
                </button>
                <button
