...
import { Users, GraduationCap, BookOpen, BarChart3, Award, TrendingUp, UserCheck, Layers, FileText } from 'lucide-react';
      ...
      <div className="min-h-screen bg-slate-900 flex">
        ...
        <div className="flex-1 ml-64 flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-500 mb-4">{error || 'No data'}</p>
            <button
              onClick={() => refetch()}
      ...
      <div className="flex-1 ml-64 px-3">
        <header className="sticky top-0 z-40 bg-slate-800/70 backdrop-blur border-b border-gray-800">
          <div className="container-custom p-4 flex items-center justify-between">
            ...
            <Link href="/admin/settings" className="text-sm text-gray-400 hover:text-white transition-colors">
              Settings →
            </Link>
          </div>
        </header>
        ...
        <main className="container-custom py-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <StatsCard title="Total Users" value={data?.users?.total ?? 0} icon={Users} trend={{ value: 12, label: 'vs last month' }} />
            <StatsCard title="Students" value={data?.users?.students ?? 0} icon={GraduationCap} accentColor="text-lime-400" />
            <StatsCard title="Instructors" value={data?.users?.instructors ?? 0} icon={UserCheck} accentColor="text-blue-400" />
            <StatsCard title="Active Users" value={data?.users?.active ?? 0} icon={TrendingUp} trend={{ value: 8 }} accentColor="text-emerald-400" />
            <StatsCard title="Graduated" value={data?.users?.graduated ?? 0} icon={Award} accentColor="text-yellow-400" />
          </div>

          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="grid sm:grid-cols-2 gap-4">
                <StatsCard title="Total Courses" value={data?.courses?.total ?? 0} icon={Layers} />
                <StatsCard title="Published Courses" value={data?.courses?.published ?? 0} icon={FileText} trend={{ value: 5, label: 'new this month' }} accentColor="text-lime-400" />
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <StatsCard title="Total Enrollments" value={data?.enrollments?.total ?? 0} icon={BookOpen} trend={{ value: 15 }} />
                <StatsCard title="Completed Enrollments" value={data?.enrollments?.completed ?? 0} icon={Award} accentColor="text-emerald-400" />
              </div>

              <RecentActivity recentUsers={data?.recentActivity?.users} recentEnrollments={data?.recentActivity?.enrollments} />
            </div>
              ...
              <div className="rounded-xl border border-gray-800 bg-slate-800/50">
                ...
                <div className="p-4 space-y-3">
                  {[
                    ...
                    <div key={stat.label} className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
